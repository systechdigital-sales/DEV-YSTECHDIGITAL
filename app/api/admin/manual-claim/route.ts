      ...
      <div className="h-screen w-screen bg-gradient-to-br from-purple-50 to-indigo-100 flex overflow-hidden">
        ...
        <Dialog open={manualAssignDialogOpen} onOpenChange={setManualAssignDialogOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Manual OTT Key Assignment</DialogTitle>
              <DialogDescription>
                Manually assign an OTT key to {selectedClaimForManualAssign?.firstName}{" "}
                {selectedClaimForManualAssign?.lastName} ({selectedClaimForManualAssign?.email})
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select value={selectedKeyForManualAssign} onValueChange={setSelectedKeyForManualAssign}>
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {availableKeys.map((key) => (
                      <SelectItem key={key._id} value={key._id}>
                        {key.product} - {key.productSubCategory} ({key.activationCode})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {availableKeys.length === 0 && <p className="text-sm text-red-600 mt-1">No available OTT keys found</p>}
              </div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={assigning || !selectedKeyForManualAssign || !manualAssignPassword}
        ...
        <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Are you sure you want to delete this {recordToDelete?.type} record for "{recordToDelete?.name}"? This
            ...
            <DialogFooter>
              ...
              <Button onClick={handleDeleteConfirm} disabled={deleting || !deletePassword} variant="destructive">
                {deleting ? (
                  <>
                    <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                    Deleting...

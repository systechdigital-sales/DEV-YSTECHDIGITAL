  ...
  <body>
      <div class="container">
          ...
          <div class="footer">
              ...
              <p style="margin: 5px 0 0 0;"> ✉️ sales.systechdigital@gmail.com</p>
      ...
      <div class="container">
          ...
          <div class="content">
              ...
              <div class="ott-code-section">
                  <p class="ott-code-label">Your Coupon Code</p>
              ...
              <div style="background: #f0f9ff; padding: 25px; border-radius: 10px; margin: 25px 0; border-left: 4px solid #3b82f6;">
                ...
                <ol style="color: #1e40af; line-height: 1.8; margin: 0; padding-left: 20px;">
                    ...
                    <li style="margin-bottom: 8px;">Enter your Coupon code:<strong>${data.ottCode}</strong></li>
...
export async function sendEmail(options: EmailOptions): Promise<boolean> {
  ...
  console.log("BytewiseTestingpoint Sending email to:", options.to)
  console.log("BytewiseTestingpoint Subject:", options.subject)
  console.log("BytewiseTestingpoint HTML content length:", options.html.length)
  console.log("BytewiseTestingpoint BCC recipients:", options.cc || "none")

  const user = process.env.GMAIL_USER
  const pass = process.env.GMAIL_APP_PASSWORD

  console.log("BytewiseTestingpoint Gmail user configured:", !!user)
  console.log("BytewiseTestingpoint Gmail password configured:", !!pass)
  console.log("BytewiseTestingpoint Gmail user value:", user ? `${user.substring(0, 3)}***@${user.split("@")[1]}` : "not set")

  if (!user || !pass) {
    console.error("BytewiseTestingpoint EMAIL CONFIGURATION ERROR: Email credentials not configured")
    console.error("BytewiseTestingpoint Please ensure GMAIL_USER and GMAIL_APP_PASSWORD environment variables are set")
    console.error("BytewiseTestingpoint GMAIL_USER exists:", !!process.env.GMAIL_USER)
    console.error("BytewiseTestingpoint GMAIL_APP_PASSWORD exists:", !!process.env.GMAIL_APP_PASSWORD)
    return false
  }

  try {
    console.log("BytewiseTestingpoint Creating Gmail transporter...")
    ...
    console.log("BytewiseTestingpoint Verifying transporter configuration...")
    await transporter.verify()
    console.log("BytewiseTestingpoint Transporter verified successfully")
    ...
    console.log("BytewiseTestingpoint Email configuration:", {
    ...
    console.log("BytewiseTestingpoint Attempting to send email...")
    const result = await transporter.sendMail(mailOptions)

    console.log("BytewiseTestingpoint Email sent successfully!")
    console.log("BytewiseTestingpoint Message ID:", result.messageId)
    console.log("BytewiseTestingpoint Response:", result.response)
    console.log("BytewiseTestingpoint BCC recipients included:", allBccRecipients)
    console.log("=== EMAIL SEND SUCCESS ===")
    return true
  } catch (error: any) {
    console.error("=== EMAIL SEND ERROR ===")
    console.error("BytewiseTestingpoint Error sending email:", error)
    console.error("BytewiseTestingpoint Error type:", typeof error)
    console.error("BytewiseTestingpoint Error name:", error?.name)
    console.error("BytewiseTestingpoint Error message:", error?.message)
    console.error("BytewiseTestingpoint Error code:", error?.code)
    console.error("BytewiseTestingpoint Error command:", error?.command)
    console.error("BytewiseTestingpoint Error stack:", error?.stack)

    if (error?.code === "EAUTH") {
      console.error("BytewiseTestingpoint AUTHENTICATION ERROR: Invalid Gmail credentials")
      console.error("BytewiseTestingpoint Check GMAIL_USER and GMAIL_APP_PASSWORD environment variables")
    } else if (error?.code === "ENOTFOUND") {
      console.error("BytewiseTestingpoint NETWORK ERROR: Cannot reach Gmail SMTP server")
      console.error("BytewiseTestingpoint Check internet connection and firewall settings")
    } else if (error?.code === "ETIMEDOUT") {
      console.error("BytewiseTestingpoint TIMEOUT ERROR: Connection to Gmail SMTP server timed out")
    } else if (error?.message?.includes("Invalid login")) {
      console.error("BytewiseTestingpoint LOGIN ERROR: Gmail login failed - check app password")
    }
...
export async function sendTemplatedEmail(emailData: EmailData): Promise<boolean> {
  console.log("BytewiseTestingpoint Sending templated email:", {
    to: emailData.to,
  ...
  if (!template) {
    console.error(`BytewiseTestingpoint Template '${emailData.template}' not found`)
    console.error("BytewiseTestingpoint Available templates:", Object.keys(emailTemplates))
    return false
  }

  console.log("BytewiseTestingpoint Template found, generating HTML...")
  const html = template(emailData.data)
  console.log("BytewiseTestingpoint Generated HTML length:", html.length)
  ...
  console.log("BytewiseTestingpoint Templated email send result:", result)
  return result
}